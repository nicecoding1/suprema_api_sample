...
$sql = "select * from jimun_api_todo where gbn='1' and procok='0' order by no limit 0,10";
$res = dbquery($sql);
while($row = dbfetch($res)) {
	$row['name'] = iconv("EUC-KR", "UTF-8", $row['name']);

	$url="http://192.168.5.5:8795/v2/users";
	$cookie="cookie.txt";

	//"status": "IN"(비활성) / "AC"(활성)
	$frdt = date('Y-m-d')."T07:00:00.00Z";
	$todt = date('Y-m-d', strtotime("+1 year"))."T18:00:00.00Z";
	...
	$_postdata = array("access_groups"=>$_access_groups, "user_group"=>$_user_group, "name"=>$row['name'], "start_datetime"=>$frdt, "expiry_datetime"=>$todt,
		"security_level"=>"DEFAULT", "status"=>"AC", "user_id"=>$row['jimunid']);
	...
	if($arr['user_id'] == $row['jimunid']) {
		//결과 회신
		$sql2 = "update jimun_api_todo set procok='1', procdt=now() where no='$row[no]'";
		dbquery($sql2);
	} else if($arr['message'] == "Duplicate User") {
		$sql2 = "update jimun_api_todo set gbn='2', procok='0' where no='$row[no]'";
		dbquery($sql2);
	}
}
