...
$sql = "select * from jimun_api_todo where gbn='2' and procok='0' and reservfg<>'1' order by no limit 0,20";
$res = dbquery($sql);
while($row = dbfetch($res)) {
	...
	$todt = date('Y-m-d', strtotime("+1 year"))."T21:00:00.00Z";
...
$sql = "select * from jimun_api_todo where gbn='2' and procok='0' and reservfg='1' order by reservdt, no limit 0,20";
$res = dbquery($sql);
while($row = dbfetch($res)) {
	$now = date('Y-m-d H:i:s');
	if($row['reservfg'] == "1" && $row['reservdt'] > $now) {
